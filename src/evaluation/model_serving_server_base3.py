...
def process_data(data: DataPayload):
    ...
    for col in cols_with_1:
        ...
        # Record the times taken for the operation
        evaluation_results[second][col] = [end_time - start_time, model_to_gpu_time, inference_time, unload_from_gpu_time]
    ...
    with open("evaluation_results.txt", "a") as f:
        for second, col_times in evaluation_results.items():
            for col, times in col_times.items():
                f.write(f"{second},{col},{times[0]},{times[1]},{times[2]},{times[3]}\n")
