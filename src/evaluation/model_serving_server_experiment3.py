...
def make_prediction(model_id: int):
    ...
    else: #current_model_on_gpu != model_id:
...
def preload_model(model_id: int):
    global current_model_on_gpu
    # unload previous model
    dl_models[current_model_on_gpu].to("cpu")
    # move desired model to GPU
...
def process_data(data: DataPayload):
    ...
    model_on_gpu_before_preloading = current_model_on_gpu

    workload_list_index = response.json()["workload_list_index"]
    workload_id_to_preload = workload_ids[workload_list_index]
    preload_model(workload_id_to_preload)

    evaluation_results[second] = {}
    for col in cols_with_1:
        start_time = time.time()
        # load the model and make prediction
        model_to_gpu_time, inference_time, unload_from_gpu_time = make_prediction(int(col))
        end_time = time.time()
        ...
        evaluation_results[second][col]["workload_id_to_preload"] = workload_id_to_preload
        evaluation_results[second][col]["model_on_gpu_before_preloading"] = model_on_gpu_before_preloading
    ...
    with open("evaluation_results.txt", "a") as f:
        for second, col_times in evaluation_results.items():
            for col, metadata in col_times.items():
                f.write(f"{second},{col},{metadata['processing_time']},{metadata['workload_id_to_preload']},{metadata['model_on_gpu_before_preloading']},{metadata['manager_time_series_time']},{metadata['model_to_gpu_time']},{metadata['inference_time']},{metadata['unload_from_gpu_time']}\n")
